...
class sourceModel {
    async validateTranslationTranslations(translations) {
        const errors = [];
    ...
    VALUES ( ?)
  `;
        return await (0, helper_1.executeQuery)(query, [data.status || "active"]);
    }
    ...
    WHERE id = ?
  `;
        return await (0, helper_1.executeQuery)(query, [data.status, data.id]);
    }
    async tipFind(data) {
  ...
  `;
    ...
    async insertMultipleMedia(files) {
        if (!files?.length) {
            return [];
        }
        const values = [];
        ...
        files.forEach((file) => {
            placeholders.push("(?, ?, ?, ?, ?, ?, ?)");
            values.push(file.file_name, file.org_name, file.url, file.path, file.mime_type, file.media_type, file.file_size);
        });
        const query = `
    INSERT INTO media (
      file_name,
  ...
  `;
        const rows = await (0, helper_1.executeQuery)(dataQuery, [...queryParams]);
        return {
            data: rows || [],
        };
    }
